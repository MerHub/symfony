...
import com.bolt.app.services.ServiceEvent;
    ...
    public indexClientPage(){
        ...
        current.setUIID("indexPage");
           ...
           new editProfilPage().show();
             ...
             new loginPage(new startPage()).show();
       });
        ...
        FontImage markertaxi = FontImage.createMaterial(FontImage.MATERIAL_CAR_REPAIR, s2, 7);
         ...
         cnt.zoom(new Coord(MyApplication.userConnect.getLatitude(), MyApplication.userConnect.getLongitude()), 18);
            ...
            //show list taxi
                ...
                // show list event
                ArrayList<Event> listEvent=ServiceEvent.getInstance().getListEvent();
                                for(int i=0; i<listEvent.size(); i++){
                    //liste.get(i)
                String txt = "oui";
                if(listEvent.get(i).getId_event()!=0 && listEvent.get(i).getLongitude1()!=0 && listEvent.get(i).getLatitude1()!=0 && !listEvent.get(i).getDescription().trim().equals("") && !listEvent.get(i).getArrivee().trim().equals("") && !listEvent.get(i).getDepart().trim().equals("") && !listEvent.get(i).getNom().trim().equals("") && listEvent.get(i).getNbr_place()!=0){
                    try {
                        Event cs=listEvent.get(i);
                        id_event=cs;
                        
                        cnt.addMarker(EncodedImage.create("/iconEvent.png"),new Coord(listEvent.get(i).getLatitude1(), listEvent.get(i).getLongitude1()), txt, "", e3->{
                           new ShowEvent(current, cs).show();
                        }); } catch (IOException ex) {
                    }
                }
                }
                ArrayList<Velo> listVelo=ServiceVelo.getInstance().getListVelo();

                                                for(int i=0; i<listVelo.size(); i++){
                    //liste.get(i)
                String txt = "oui";
                if(listVelo.get(i).getId()!=0 && !listVelo.get(i).getAdresse().trim().equals("") && !listVelo.get(i).getType().trim().equals("") && 
                        !listVelo.get(i).getPhoto().trim().equals("") && listVelo.get(i).getPrix()!=0 && listVelo.get(i).getLatitude()!=0 && listVelo.get(i).getLongitude()!=0){
                    try {
                        Velo cs=listVelo.get(i);
                        cnt.addMarker(EncodedImage.create("/iconVelo.png"),new Coord(listVelo.get(i).getLatitude(), listVelo.get(i).getLongitude()), txt, "", e3->{
                           new ShowVelo(current, cs).show();
                        }); } catch (IOException ex) {
                    }
                }
                }
        ...
        cnt.addTapListener(e->{
            ...
            try {
                ...
                if(listVelo.get(i).getId()!=0 && !listVelo.get(i).getAdresse().trim().equals("") && !listVelo.get(i).getType().trim().equals("") && 
                    ...
                    try {
                        Velo cs=listVelo.get(i);
                        cnt.addMarker(EncodedImage.create("/iconVelo.png"),new Coord(listEvent.get(i).getLatitude1(), listEvent.get(i).getLongitude1()), txt, "", e3->{
                           new ShowVelo(current, cs).show();
                        }); } catch (IOException ex) {
                    }
                }
